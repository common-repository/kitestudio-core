...
/*-----------------------------------------------------------------------------------*/
if ( ! function_exists( 'kite_sc_iconbox_noborder' ) ) {
	function kite_sc_iconbox_noborder( $atts, $content = null ) {
		extract(
			shortcode_atts(
				array(
					'title'                => '',
		...
		<div id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( implode( ' ', $class ) ); ?>"
			...
			<div class="icon">
				<span class="glyph <?php echo esc_attr( $icon ); ?>"
					 ...
					 style="
														 <?php
															if ( strlen( $icon_border_color ) ) {
																?>
		 border-color: <?php echo esc_attr( $icon_border_color ); ?>; background-color: <?php echo esc_attr( $icon_border_color ); ?>;<?php } ?> color: <?php echo esc_attr( $icon_color ); ?>;" <?php } ?>></span>
			</div>
					...
					if ( strlen( $link['url'] ) ) {
						...
						<div class="more-link">
							<a href="<?php echo esc_url( $link['url'] ); ?>"
												...
												if ( strlen( $content_color ) ) {
													?>
								 style="color: <?php echo esc_attr( $content_color ); ?>;" <?php } ?> <?php
									if ( $link['target'] != '' ) {
										?>
		target="<?php echo esc_attr( $link['target'] ); ?>"<?php } ?>>[ <?php echo esc_html( $link['title'] ); ?> ]</a>
				...
				} elseif ( ! empty( $url ) && ! empty( $elementor_link_title ) ) {
					$link['url']    = $url;
